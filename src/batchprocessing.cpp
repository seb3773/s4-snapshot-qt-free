...
Batchprocessing::Batchprocessing(Settings *settings)
    : settings(settings),
      work(settings)
{
    ...
    if (!settings->checkCompression()) {
        qCritical().noquote() << QObject::tr("Error")
...
#ifdef UNIT_TESTS
Batchprocessing::Batchprocessing(Settings *settings, bool /*unitTestNoRun*/)
    : settings(settings),
      work(settings)
{
}
...
QString Batchprocessing::colorizeDiffAnsi(const QString &diff)
{
...
bool Batchprocessing::resetCustomExcludesCli(const QString &configuredPath, const QString &sourcePath) const
{
    ...
    if (FileCpp::exists(configuredPath.toStdString())) {
        ...
        if (!FileCpp::remove(configuredPath.toStdString())) {
            qWarning().noquote() << QObject::tr("Could not remove existing exclusion file at %1.").arg(configuredPath);
            return false;
        }
    }
...
{
    checkUpdatedDefaultExcludesCliWithIo(stdin, stdout);
}

void Batchprocessing::checkUpdatedDefaultExcludesCliWithIo(FILE *in, FILE *out)
{
    QString diffOutput;
    ...
    const QString configuredPath = settings->snapshotExcludesPath;
    const QString sourcePath = settings->getExcludesSourcePath();
    qDebug().noquote() << QObject::tr("Detected newer exclusion file at %1 compared to %2. Prompting for action.")
                              .arg(sourcePath, configuredPath);

    const QString showOptionKey = QObject::tr("s", "CLI excludes prompt: single-letter shortcut for 'show diff'");
    const QString useOptionKey = QObject::tr("u", "CLI excludes prompt: single-letter shortcut for 'use updated default'");
    const QString keepOptionKey = QObject::tr("k", "CLI excludes prompt: single-letter shortcut for 'keep custom (update timestamp)'");
    const QString quitOptionKey = QObject::tr("q", "CLI excludes prompt: single-letter shortcut for 'quit'");

    const QString showOptionText = QObject::tr("show diff", "CLI excludes prompt option label");
    const QString useOptionText = QObject::tr("use updated default", "CLI excludes prompt option label");
    const QString keepOptionText = QObject::tr("keep custom (update timestamp)", "CLI excludes prompt option label");
    const QString quitOptionText = QObject::tr("quit", "CLI excludes prompt option label");

    const QString optionPrompt =
        QObject::tr("[%1]%2  [%3]%4  [%5]%6  [%7]%8: ")
    ...
    while (true) {
        (void)StdioCpp::write(out,
                              QObject::tr("The exclusion file at %1 is newer than your configured file at %2.")
                                  .arg(sourcePath, configuredPath)
                                  .toLocal8Bit()
                                  .toStdString());
        (void)StdioCpp::write(out, "\n");
        (void)StdioCpp::write(out, optionPrompt.toLocal8Bit().toStdString());
        (void)StdioCpp::flush(out);

        const QString response = QString::fromLocal8Bit(StdioCpp::readLine(in).c_str()).trimmed();

        if (response.compare(showOptionKey, Qt::CaseInsensitive) == 0
            || response.compare(showOptionText, Qt::CaseInsensitive) == 0) {
            (void)StdioCpp::write(out, colorizeDiffAnsi(diffOutput).toLocal8Bit().toStdString());
            (void)StdioCpp::flush(out);
            continue;
        }
        ...
        if (response.compare(useOptionKey, Qt::CaseInsensitive) == 0
            ...
            if (resetCustomExcludesCli(configuredPath, sourcePath)) {
                qDebug().noquote() << QObject::tr("Reverted to updated default exclusion file.");
            }
            return;
        }
        ...
        if (response.compare(quitOptionKey, Qt::CaseInsensitive) == 0
            ...
            qDebug() << QObject::tr("Leaving custom exclusion file unchanged.");
        ...
        (void)StdioCpp::write(out, QObject::tr("Invalid choice. Please select again.").toLocal8Bit().toStdString());
        (void)StdioCpp::write(out, "\n");
    }
}
...
void Batchprocessing::checkNvidiaGraphicsCard()
{
    if (CommandRunner::run("glxinfo | grep -q NVIDIA")) {
        qDebug() << QObject::tr("This computer uses an NVIDIA graphics card. Are you planning to use the "
        ...
        if (response == "yes" || response == "y") {
            ...
            qDebug() << QObject::tr("Note: If you use the resulting ISO on a computer without an NVIDIA card, "
                           "you will likely need to remove 'xorg=nvidia' from the boot options.");
        } else {
            qDebug() << QObject::tr("Note: If you use the resulting ISO on a computer with an NVIDIA card, "
